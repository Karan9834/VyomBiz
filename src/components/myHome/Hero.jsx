...
const Hero = () => {
  ...
  const topButtons = [
    ...
    { label: "Lawyer Consultation", path: "/lawyers-services" },
  ];
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3 mt-6">
            {topButtons.map((button, index) => (
              <Link
                key={index}
                to={button.path}
                className="px-4 md:px-5 py-2 bg-[#FFE90A] border border-[#FFE90A]/60 rounded-full text-[#072b47] text-xs md:text-sm font-bold hover:bg-white hover:border-white hover:text-[#072b47] transition-all duration-300 shadow-md hover:shadow-2xl active:scale-95 hover:scale-110 cursor-pointer block sm:inline-block"
              >
                {button.label}
              </Link>
            ))}
